"use client";
import { Plus, LayoutGrid, List, Flag } from "lucide-react";
...
const EMPTY_TASK_FORM = { title: "", desc: "", priority: "medium", goalId: "", dueDate: "", assigneeId: "" };

export default function TasksPage() {
  const {
    activeWorkspace, addTask, deleteTask, updateTask,
    goals, loadGoals, loadTasks, moveTask,
    setTaskView, taskView, tasks, tasksLoading, users,
  } = useAppStore();
  ...
  const createGoalId = useWatch({ control: createForm.control, name: "goalId" });
  const createAssigneeId = useWatch({ control: createForm.control, name: "assigneeId" });
  ...
  const editAssigneeId = useWatch({ control: editForm.control, name: "assigneeId" });
  ...
  const openEdit = (task, col) => {
    ...
    editForm.reset({
      ...
      assigneeId: task.assignee?.id && task.assignee.id !== "unassigned" ? task.assignee.id : "",
    });
  };
  ...
  return (
    <>
      <div className="space-y-5 animate-slide-in">
        <Header
          title="Tasks"
          subtitle={tasksLoading ? "Loading tasks..." : `${totalTasks} tasks across ${COLUMNS.length} columns`}
        />
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex gap-1 bg-(--surface-2) p-1 rounded-xl">
            <button
              ...
              className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all",
                taskView === "kanban" ? "bg-(--surface) text-foreground shadow-sm" : "text-(--text-muted)")}
            >
            ...
            <button
              ...
              className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all",
                taskView === "list" ? "bg-(--surface) text-foreground shadow-sm" : "text-(--text-muted)")}
            >
          ...
          <div className="ml-auto">
            <Button icon={<Plus className="w-4 h-4" />} onClick={() => handleAddTask("todo")}>New Task</Button>
          </div>
        </div>

        {taskView === "kanban" ? (
          <KanbanBoard tasks={tasks} onMove={handleMove} onAddTask={handleAddTask} onEdit={openEdit} onDelete={confirmDelete} />
        ) : (
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={createForm.handleSubmit(handleCreate)} loading={createForm.formState.isSubmitting}>
              Create Task
            </Button>
          </>
        }
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={editForm.handleSubmit(handleEdit)} loading={editForm.formState.isSubmitting}>
              Save Changes
            </Button>
          </>
        }

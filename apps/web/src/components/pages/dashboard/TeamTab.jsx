...
export default function TeamTab({ users, contributions = [] }) {
  const maxContrib = Math.max(
    ...contributions.map((m) => m.tasks + m.goals + m.announcements),
    1,
  );

  const chartData = contributions.map((m) => ({
    name: m.name?.split(" ")[0] ?? "?",
    tasks: m.tasks,
    goals: m.goals,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <Card className="lg:col-span-3">
          ...
          {contributions.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)] py-8 text-center">No contribution data yet</p>
          ) : (
        ...
        <Card className="lg:col-span-2">
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -24 }} barSize={9}>

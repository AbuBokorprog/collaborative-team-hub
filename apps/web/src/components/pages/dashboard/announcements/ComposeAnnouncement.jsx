    ...
    return (
      <Card>
        <div className="flex items-center gap-3">
          ...
          <Button icon={<Plus className="w-4 h-4" />} onClick={() => setComposing(true)}>
            New Post
          </Button>
        </div>
      </Card>
  ...
  return (
    <Card className="border-[var(--accent)]/30">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-[var(--text-primary)]">New Announcement</h3>
        <button
      ...
      <div className="space-y-3">
        ...
        />
        <RichEditor value={content} onChange={(value) => setValue("content", value)} />
        ...
        <div className="flex gap-2 pt-1">
          <Button variant="secondary" onClick={() => setComposing(false)}>Discard</Button>
          <Button onClick={onPost} disabled={!content?.trim()} loading={isSubmitting}>
            Post Announcement
          </Button>
        </div>
      </div>

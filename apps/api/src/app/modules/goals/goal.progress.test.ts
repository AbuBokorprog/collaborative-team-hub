...
const testProgressCalculation = () => {
  ...
  const tasks1 = []
  const expected1 = 0
  const actual1 = tasks1.length === 0 ? 0 : Math.round((tasks1.filter(t => t.status === 'DONE').length / tasks1.length) * 100)
  console.log(`Test 1 - No tasks: ${actual1 === expected1 ? 'PASS' : 'FAIL'} (Expected: ${expected1}, Actual: ${actual1})`)
  ...
  const expected2 = 100
  const actual2 = Math.round((tasks2.filter(t => t.status === 'DONE').length / tasks2.length) * 100)
  console.log(`Test 2 - All done: ${actual2 === expected2 ? 'PASS' : 'FAIL'} (Expected: ${expected2}, Actual: ${actual2})`)
  ...
  const expected3 = 50
  const actual3 = Math.round((tasks3.filter(t => t.status === 'DONE').length / tasks3.length) * 100)
  console.log(`Test 3 - Half done: ${actual3 === expected3 ? 'PASS' : 'FAIL'} (Expected: ${expected3}, Actual: ${actual3})`)
  ...
  const expected4 = 33
  const actual4 = Math.round((tasks4.filter(t => t.status === 'DONE').length / tasks4.length) * 100)
  console.log(`Test 4 - One third done: ${actual4 === expected4 ? 'PASS' : 'FAIL'} (Expected: ${expected4}, Actual: ${actual4})`)
